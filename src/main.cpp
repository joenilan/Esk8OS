...
// ==========================================
// COLOR THEMES
// Palette table (RGB888) selectable from Settings > THEME, persisted in NVS.
// applyTheme() recomputes the COL_* globals; a drawStaticFrame() repaints with
// them. NOTE: the ST7789 reads blue-leaning purples as blue, so accents are
// magenta-shifted on purpose (the "CAM" accent is the verified-good value).
// ==========================================
int gThemeIdx = 0;   // loaded from NVS in setup()

const Theme THEMES[] = {
    // name        bg            border        dim            label          white            green          red            accent         yellow         orange
...
void setup() {
    ...
    applyTheme(gThemeIdx);   // palette from the selected theme (loaded above)
        ...
        if (currentPage == PAGE_SETTINGS) {
            switch (settingsCursor) {
                ...
                case SET_THEME:
                    ...
                    applyTheme(gThemeIdx);     // recompute palette
                    drawStaticFrame();         // repaint with the new colors
                    break;
                case SET_CELLS:
                    BATTERY_CELLS_COUNT++;
