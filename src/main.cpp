...
void drawHudSmallMetric(int x, int y, int w, const char* label, String value, uint16_t color) {
    GFX->drawRect(X0 + x, y, w, 36, COL_BORDER);

    GFX->setFont(&fonts::Font0);
    GFX->setTextDatum(TC_DATUM);
    GFX->setTextColor(COL_DIM);
    GFX->drawString(label, X0 + x + w / 2, y + 3);

    GFX->setFont(&fonts::FreeSansBold12pt7b);
    GFX->setTextDatum(MC_DATUM);
    GFX->setTextColor(color);
    GFX->drawString(value, X0 + x + w / 2, y + 24);
...
void updateHud() {
    ...
    // Reclaim the old bottom cell/page-dot space on the HUD. The bottom status
    // bar still starts at y=300, so the ride content gets almost the whole panel.
    GFX->fillRect(X0, 18, UI_W, 278, COL_BG);

    int spdInt = (int)round(useMph ? currentSpeedMph : currentSpeedKmh);

    // Giant speed, scaled up and anchored by its BOTTOM just above the unit
    // label. Bottom-anchoring keeps the gap to MPH/KM-H tight and identical no
    // matter how tall the (scaled) number is — taller numbers grow upward into
    // the empty area instead of leaving a gap above the label. Scale is reduced
    // for more digits so the number still fits the 170px UI width.
    float hudSpeedScale = (spdInt >= 100) ? 1.20f : ((spdInt >= 10) ? 1.55f : 1.85f);
    GFX->setFont(&BebasNeue80pt7b);
    GFX->setTextSize(hudSpeedScale);
    GFX->setTextDatum(BC_DATUM);
    GFX->setTextColor(COL_WHITE);
    GFX->drawString(String(spdInt), X0 + UI_W / 2, 128);
    GFX->setTextSize(1.0f);

    GFX->setFont(&fonts::FreeSansBold9pt7b);
    GFX->setTextDatum(MC_DATUM);
    GFX->setTextColor(COL_LABEL);
    GFX->drawString(useMph ? "MPH" : "KM/H", X0 + UI_W / 2, 138);

    // Big HUD battery cells. Same squared style as the bottom strip, but larger
    // and only shown on this page.
    drawBatteryCellsRow(162, 16, true);

    GFX->setFont(&fonts::FreeSansBold12pt7b);
    GFX->setTextDatum(MC_DATUM);
    GFX->setTextColor(COL_WHITE);
    GFX->drawString(String(currentBatteryPercent) + "%", X0 + UI_W / 2, 194);

    // Main riding/debug metrics, shifted down into the reclaimed area.
    int watts = (int)round(max(0.0f, peakWatts));
    float rangeDisplay = useMph ? remainingRangeKm * 0.621371f : remainingRangeKm;

    drawHudSmallMetric(4, 222, 78, "WATTS", String(watts), wattColor(watts));
    drawHudSmallMetric(88, 222, 78, "VOLTS", String(currentVoltage, 1), battColor(currentBatteryPercent));

    String rangeUnit = useMph ? "mi" : "km";
    drawHudSmallMetric(4, 262, 78, "RANGE", String(rangeDisplay, 1) + rangeUnit, COL_WHITE);

    float hottest = max(currentMotorTemp, currentEscTemp);
    drawHudSmallMetric(88, 262, 78, "TEMP", String((int)round(hottest)) + "C",
